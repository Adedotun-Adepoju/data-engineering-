...
green_taxi_url_prefix = 'https://d37ci6vzurychx.cloudfront.net/trip-data/green_tripdata_'

file_suffix = '{{ execution_date.strftime(\'%Y-%m\') }}.parquet'

green_url_template = green_taxi_url_prefix + file_suffix
green_file_template = f'{path_to_local_home}/green_{file_suffix}'
...
with DAG(
    ...
    download_dataset_task = BashOperator(
        ...
        bash_command=f"curl -SS { green_url_template } -o {green_file_template}"
    )
    ...
    upload_to_gcs_task = PythonOperator(
        ...
        op_kwargs = {
            "bucket": BUCKET,
            "local_file": green_file_template,
            "object_name": f"green_taxi_data/{file_suffix}"
        }
    )

    clean_up_task = BashOperator(
        task_id="clean_up_task",
        bash_command=f"rm { green_file_template }"
    )
